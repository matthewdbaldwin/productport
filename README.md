...
| `HUBPORT_LIFECYCLE_SECRET` | HMAC secret for inbound HubPort → ProductPort SSO-lifecycle events (`POST /api/sso/lifecycle/event`); blank fails closed (503) |
| `ALLOW_UNSIGNED_LIFECYCLE` | Local-dev-only: skip lifecycle HMAC when no secret is provisioned. The API refuses to boot with it set under `NODE_ENV=production` |
...
ProductPort is SSO/JIT: on every login the auth middleware re-resolves the user's role from the SSO claim and upserts the local `User` row. Inbound lifecycle events (`POST /api/sso/lifecycle/event`) go through microport-auth's shared receiver (`createLifecycleReceiver`, adopted in hubport#133), which owns the signature check (`x-hubport-signature` over `${X-Lifecycle-Event-Id}.${rawBody}`), `LifecycleEvent` contract validation, the `UserLifecycleEvent` audit row, dedup on `X-Lifecycle-Event-Id`, the atomic claim, ordering and the 5-minute reclaim sweep (started in `src/server.js`). ProductPort supplies the policy in `src/lib/lifecycleAction.js` and acts on the account flag — `disable` deactivates the user (so an offboarded account loses access before its token expires), `grant`/`reactivate` re-enable it, and HubPort's `fleetSuperuser` flag is written when the event carries it. A `grant`/`reactivate` for an email with **no local row creates it** (fleet create-on-grant decision, 2026-08-19) when the event's role maps — placeholder name from the email local-part; the real name and role re-sync from the SSO claim at first login. For existing rows, `grant` / `revoke` write no role here; the role takes effect on the next login. Every user write is conditional on `User.lifecycleSeq`, so a late or retried older event can't overwrite a newer one. Log lines on this path never carry the subject's email.
